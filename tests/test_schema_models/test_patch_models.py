...
class TestPatch:
    ...
    def test_patch_with_multiple_patches_same_type(self):
        ...
        patch = Patch(
            project_patches=[project_patch1, project_patch2],
            task_patches=[task_patch1, task_patch2]
        )
    ...
    def test_patch_edge_cases(self):
        ...
        # Many patches
        many_project_patches = [
            ProjectPatch(op=Op.CREATE, name=f"Project {i}")
            for i in range(10)
        ]
        many_task_patches = [
            TaskPatch(op=Op.CREATE, project_id=str(uuid4()), title=f"Task {i}")
            for i in range(10)
        ]
        
        patch = Patch(
            project_patches=many_project_patches,
            task_patches=many_task_patches
        )
