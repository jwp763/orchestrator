    ...
    def _get_model(self, provider: str, model_name: str) -> Model:
        ...
        if cache_key not in self._models:
            ...
            if provider == "anthropic":
                self._models[cache_key] = AnthropicModel(model_name=model_name, api_key=api_key)
            elif provider == "openai":
                self._models[cache_key] = OpenAIModel(model_name=model_name, api_key=api_key)
            elif provider == "gemini":
                self._models[cache_key] = GeminiModel(model_name=model_name, api_key=api_key)
            elif provider == "xai":
                # Using Groq as a placeholder - you'd implement XAI model
                self._models[cache_key] = GroqModel(model_name=model_name, api_key=api_key)
            else:
