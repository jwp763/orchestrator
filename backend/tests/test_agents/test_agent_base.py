...
class TestAgentBase:
    ...
    def mock_settings(self):
        ...
        with patch("src.agent.base.get_settings") as mock_get_settings:
            ...
            mock_settings.providers.providers = {"anthropic": Mock(default="claude-3-haiku-20240307")}
    ...
    def test_agent_initialization_with_custom_params(self, mock_settings):
        ...
        agent = ConcreteAgent(provider="openai", model_name="gpt-4", max_retries=5, retry_delay=2.0)
    ...
    def test_get_model_creates_anthropic_model(self, mock_anthropic, concrete_agent, mock_settings):
        ...
        # Now it should be called without api_key parameter (uses environment variable)
    ...
    async def test_call_llm_with_retry_success_first_attempt(self, concrete_agent):
        ...
        with patch.object(agent, "_create_agent", return_value=mock_agent):
            result = await agent._call_llm_with_retry("Create a project", "You are a helpful assistant", ProjectPatch)
        ...
        with patch.object(agent, "_create_agent", return_value=mock_agent):
            with patch.object(agent, "_sleep_with_backoff") as mock_sleep:
                result = await agent._call_llm_with_retry(
                    "Create a project", "You are a helpful assistant", ProjectPatch
                )
    ...
    async def test_call_llm_with_retry_max_retries_json_error(self, concrete_agent):
        ...
        with patch.object(agent, "_create_agent", return_value=mock_agent):
            with patch.object(agent, "_sleep_with_backoff"):
                with pytest.raises(JSONParsingError, match="Failed to parse JSON after 3 attempts"):
                    await agent._call_llm_with_retry("Create a project", "You are a helpful assistant", ProjectPatch)
    ...
    def test_sync_wrapper_methods(self, concrete_agent):
        ...
        with patch("asyncio.new_event_loop") as mock_new_loop, patch("asyncio.set_event_loop") as mock_set_loop:
            ...
            # Mock the async method to avoid actual async call
            with patch.object(agent, 'get_diff', return_value=mock_patch):
    ...
    def test_call_llm_with_retry_sync(self, concrete_agent):
        ...
        with patch("asyncio.new_event_loop") as mock_new_loop, patch("asyncio.set_event_loop") as mock_set_loop:
            ...
            # Mock the async method to avoid actual async call
            with patch.object(agent, '_call_llm_with_retry', return_value=mock_patch):
                result = agent._call_llm_with_retry_sync("Create a project", "System prompt", ProjectPatch)
    ...
    def test_create_agent(self, mock_agent_class, mock_settings):
        ...
        mock_agent_class.assert_called_once_with(
            model=mock_model, output_type=str, system_prompt="Test system prompt"
        )
