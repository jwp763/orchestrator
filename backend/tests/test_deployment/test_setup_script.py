...
class TestNodeDependenciesInstallation:
    ...
    @patch('subprocess.run')
    def test_install_node_dependencies_success(self, mock_run):
        ...
        calls = mock_run.call_args_list
        assert any("npm install" in str(call) for call in calls)
    
    @patch('subprocess.run')
    def test_install_node_dependencies_failure(self, mock_run):
    ...
    @patch('subprocess.run')
    def test_install_node_dependencies_checks_node_version(self, mock_run):
        """Test that Node installation checks version first."""
        mock_run.return_value = MagicMock(returncode=0, stdout="v18.0.0")
        
        result = install_node_dependencies()
        
        # Should check Node version before installing
        calls = mock_run.call_args_list
        assert any("node --version" in str(call) for call in calls)
...
class TestEnvironmentFilesCreation:
    ...
    def test_create_environment_files_from_examples(self, mock_open, mock_exists):
        ...
        # Should create files from examples
...
class TestDatabaseSetup:
    ...
    def test_setup_database_runs_migrations(self, mock_run):
        ...
        # Should run migration commands
        calls = mock_run.call_args_list
        migration_commands = ["alembic", "migrate", "upgrade"]
        assert any(any(cmd in str(call) for cmd in migration_commands) for call in calls)
...
class TestInitialTests:
    ...
    def test_run_initial_tests_frontend(self, mock_run):
        ...
        # Should run npm test
        calls = mock_run.call_args_list
        assert any("npm test" in str(call) for call in calls)
...
class TestCompleteSetupFlow:
    ...
    @patch('scripts.setup_development.install_python_dependencies')
    @patch('scripts.setup_development.install_node_dependencies')
    @patch('scripts.setup_development.create_environment_files')
    @patch('scripts.setup_development.setup_database')
    def test_complete_setup_flow_success(self, mock_db, mock_env, mock_node, mock_python):
