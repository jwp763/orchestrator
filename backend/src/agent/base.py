...
class AgentBase(ABC):
    ...
    def _get_model(self) -> Model:
        ...
        if cache_key not in self._model_cache:
            ...
            # Set environment variable for the provider (pydantic-ai reads from env)
            import os
            if self.provider == "anthropic":
                os.environ['ANTHROPIC_API_KEY'] = api_key
                self._model_cache[cache_key] = AnthropicModel(model_name=self.model_name)
            elif self.provider == "openai":
                os.environ['OPENAI_API_KEY'] = api_key
                self._model_cache[cache_key] = OpenAIModel(model_name=self.model_name)
            elif self.provider == "gemini":
                os.environ['GOOGLE_API_KEY'] = api_key
                self._model_cache[cache_key] = GeminiModel(model_name=self.model_name)
            elif self.provider == "xai":
                # Using Groq as a placeholder for XAI
                os.environ['GROQ_API_KEY'] = api_key
                self._model_cache[cache_key] = GroqModel(model_name=self.model_name)
            else:
    ...
    def _create_agent(self, system_prompt: str) -> Agent[None, str]:
        ...
        return Agent(model=model, output_type=str, system_prompt=system_prompt)

    async def _call_llm_with_retry(self, prompt: str, system_prompt: str, response_type: Type[T]) -> T:
        """
                ...
                try:
                    ...
                    if attempt < self.max_retries:
                        ...
                        raise JSONParsingError(
                            f"Failed to parse JSON after {self.max_retries + 1} attempts. " f"Last error: {e}"
                        )
                ...
                try:
                    ...
                    if attempt < self.max_retries:
                        ...
                        raise ValidationError(
                            f"Failed validation after {self.max_retries + 1} attempts. " f"Last error: {e}"
                        )
    ...
    def _call_llm_with_retry_sync(self, prompt: str, system_prompt: str, response_type: Type[T]) -> T:
        """
        ...
        try:
            return loop.run_until_complete(self._call_llm_with_retry(prompt, system_prompt, response_type))
        finally:
            loop.close()
